...
/// For every instrument in the <see cref="Universe"/> it opens the stored quote, trade, and depth streams
...
public sealed class StoreMarketDataFeed : IMarketDataFeed
{
    ...
    private async IAsyncEnumerable<MarketEvent> Depth(
        InstrumentSpec inst, DateTime from, DateTime to, [EnumeratorCancellation] CancellationToken ct)
    {
        await foreach (var snapshot in _store.ReadDepthAsync(inst.Id, from, to, ct).WithCancellation(ct))
            yield return MarketEvent.OfDepth(inst.Id, snapshot.TimestampUtc, snapshot);
    }
}
